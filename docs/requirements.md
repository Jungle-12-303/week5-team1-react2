...
프로젝트는 브라우저에서 동작하는 `단일 루트 엔트리 기반의 상태 기반 다중 페이지 SPA` 형태의 데모를 제공해야 하며, 다음 목표를 만족해야 한다.
...
- 브라우저에서 실행 가능하다.
- `src/app/main.js`와 `index.html`을 기준으로 하나의 루트 앱으로 시작한다.
- 하나의 HTML 셸을 유지하되, 사용자에게는 여러 기능 페이지로 분리된 서비스처럼 보여야 한다.
- 페이지 전환은 루트 상태 기반으로 처리할 수 있다.
...
데모 애플리케이션의 `여러 페이지`는 아래 의미로 정의한다.

- 엔트리포인트와 HTML 셸은 하나다.
- 루트 `FunctionComponent` 하나가 전체 애플리케이션 상태를 소유한다.
- `currentPage` 또는 이에 준하는 루트 상태가 현재 화면을 결정한다.
- 페이지 전환은 새로운 앱 mount가 아니라 같은 루트 앱의 재렌더로 처리한다.
- 페이지별 데이터는 별도 독립 store가 아니라 루트 상태와 파생 데이터로 관리한다.
- 사용자는 `대시보드 / 루틴 관리 / 인사이트 / 설정`처럼 기능이 분리된 페이지 구조로 인식해야 한다.
...
- 여러 기능 페이지는 별도 엔트리 파일이 아니라 같은 루트 앱 안에서 상태 기반 전환으로 구현해야 한다.
...
- 페이지 전환 시 추가 mount를 만들면 안 되며, 기존 루트 앱의 상태 변경으로 화면을 바꿔야 한다.
...
- 데모는 단일 루트 엔트리 기반의 상태 기반 다중 페이지 SPA로 동작한다.
